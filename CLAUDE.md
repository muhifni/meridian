...
Integrasi sudah ada di `lessons.js` (recordPerformance). Pelajaran sekarang punya feedback loop → semakin "pintar" seiring waktu (sesuai konsep Swarm Intelligence).
...
|--------|--------|-------------|
| `tools/dlmm.js:573` | `addLiquidity()` | Tidak kirim TX, return detail posisi yang *akan* di-deploy |
| `tools/dlmm.js:1460` | `claimFees()` | Tidak claim fee |
| `tools/dlmm.js:1506` | `closePosition()` | Tidak close posisi |
| `tools/wallet.js:153` | `swapToken()` | Tidak swap token |

### Yang tetap jalan saat DRY_RUN

- **Screening** — pencarian pool tetap aktif (`index.js:406-407` — cek SOL balance di-skip)
- **Balance check** di executor di-skip (`executor.js:793`) — jadi tidak perlu punya SOL
- **Analisis & decision-making** — agent tetap bisa evaluate pool, hitung range, dll
- **HiveMind** — melaporkan status `dryRun: true` ke central (`hivemind.js:181`)

### Log startup

```
index.js:45 → log "Mode: DRY RUN" atau "Mode: LIVE"
```

**Kesimpulan:** DRY_RUN mode aman untuk testing — agent jalan full (screening, analisis, decision) tapi tidak ada transaksi on-chain yang dikirim.
